# TimeRadar Store Listing

## Positioning

TimeRadar is an Arabic-first focus companion for Saudi professionals who need to start focused work quickly, protect deep-work blocks, and review daily progress through a simple radar signal.

Primary audience: Saudi professionals, freelancers, consultants, founders, designers, developers, and managers who want a clean focus timer without planning noise.

TimeRadar stays free forever. The optional $1.99 Supporter Pack supports the developer and unlocks a warmer visual theme as a small thank-you; it does not lock timer, task, stats, notification, Arabic, English, or RTL features.

## Arabic

App name: TimeRadar - رادار التركيز

Subtitle: تركيز مهني بلا تشتيت

Short description:
ابدأ جلسة تركيز خلال ثوان، تابع إشارة تركيزك اليومية، وراجع تقدمك المهني بدون تعقيد.

Long description:
TimeRadar يساعدك على تحويل وقت العمل إلى جلسات تركيز واضحة وهادئة. افتح التطبيق، ابدأ جلسة تركيز سريعة، واترك المؤقت والتنبيهات والإحصائيات الخفيفة تساعدك على حماية وقتك وإنجاز أعمالك اليومية.

يبقى TimeRadar مجانيًا دائمًا. حزمة الداعم الاختيارية بقيمة 1.99 دولار تدعم المطوّر وتفتح مظهرًا بصريًا أدفأ، بدون قفل أي ميزة أساسية.

Key benefits:
- جلسة تركيز سريعة بضغطة واحدة
- مؤقت تركيز مع استراحات قصيرة وطويلة
- إشارة رادار يومية تقيس ثبات تركيزك
- إحصائيات بسيطة لليوم والأسبوع والشهر والسنة
- تجربة عربية وإنجليزية مع دعم RTL
- حزمة داعم اختيارية لا تغيّر مجانية التطبيق

## English

App name: TimeRadar

Subtitle: Professional focus, without the noise.

Short description:
Start a focus block in seconds, strengthen today’s radar signal, and review calm progress stats.

Long description:
TimeRadar is a focused work timer for professionals who want less setup and more execution. Start a quick focus block, follow a simple focus/break rhythm, and see your daily radar signal grow as you complete focused work.

TimeRadar stays free forever. The optional $1.99 Supporter Pack supports the developer and unlocks a warmer visual theme as a small thank-you; all core focus features remain free.

Key benefits:
- One-tap quick focus block
- Focus timer with planned breaks
- Daily radar signal for visible momentum
- Lightweight progress stats
- Arabic-first experience with English support
- Optional Supporter Pack without locked productivity features

## Screenshot Set

1. Arabic Home: "تركيز مهني بلا تشتيت" with the daily radar signal above the timer.
2. Arabic Timer: a 25-minute focus block ready to start.
3. Arabic Stats: daily/weekly professional focus progress and distribution.
4. Arabic Supporter Pack: free-forever promise with the optional supporter visual theme.
5. English Home: Professional focus, without the noise.

## Store Experiments

- Test Arabic subtitle against "مؤقت تركيز للعمل اليومي".
- Test first screenshot with radar signal versus active timer.
- Segment campaigns for Saudi professionals during project deadlines, client work, and busy operational weeks.
